...
public class HangulUtil {
    ...
    public static int getOneHangulJasoSize(int startIdx, List<String> jasoList) {
        int remainJasoSize = jasoList.size() - startIdx;

        if (remainJasoSize == 1) {
            return -1;
        }

        if (remainJasoSize == 2 || remainJasoSize == 3) {
            return remainJasoSize;
        }

        // 초성이나 종성은 겹치는 문자가 존재하기 때문에
        // 다음 글자의 중성을 이용하여 한글자의 사이즈를 검사한다.
        String strJungSung = new String(JUNG_SUNG);
        String strChar = jasoList.get(startIdx + 3);
    ...
    public static int getChoSungIndex(int startIdx, List<String> jasoList) {
        String strChoSung = new String(CHO_SUNG);
        String strChoSungChar = jasoList.get(startIdx);

        return strChoSung.indexOf(strChoSungChar);
    }

    public static int getJungSungIndex(int startIdx, List<String> jasoList) {
        String strJungSung = new String(HangulUtil.JUNG_SUNG);
        String strJungSungChar = jasoList.get(startIdx + 1);

        return strJungSung.indexOf(strJungSungChar);
    }

    public static int getJongSungIndex(int startIdx, List<String> jasoList) {
        String strJongSung = new String(HangulUtil.JONG_SUNG);
        String strJongSungChar = jasoList.get(startIdx + 2);
